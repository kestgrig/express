...
export const serviceMembersRouter = express.Router({
    mergeParams: true, //parametras kad pasikartojantis parametras gali buti tarp tevinio ir vaikijnio elemento
});
...
serviceMembersRouter.get('/:memberName', (req, res) => {
    ...
    if (!servicesData.includes(serviceName)) { //jei nera tokio parametro
        return res.send('Services page: such service is not recognized...');
    }

    if (!members.includes(memberName)) {  //jei nera tokio parametro
